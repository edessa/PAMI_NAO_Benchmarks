...
checkpoint = torch.load('./weights/time_maps_rgb_overfit.pt')
...
#mask_val_nao_data = sorted(glob.glob('./train/masks_nao/*'))
#mask_val_cont_data = sorted(glob.glob('./train/masks_cont/*'))

train_save_dataset = CustomDataset(image_data, mask_nao_data, mask_cont_data, train=True)
train_save_loader = torch.utils.data.DataLoader(train_save_dataset, batch_size=16, shuffle=False, num_workers=1)
